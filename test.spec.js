...
describe('LunarCraft', () => {
    ...
    test('executing ["f","f","f","f","f","f"] (checking boundary function working properly or not', () => {
        const commands = ["f", "f", "f", "f", "f", "f"];
        craft.executeCommands(commands);
        expect(craft.getPosition()).toEqual({ x: 0, y: 5, z: 0 });
        expect(craft.getDirection()).toBe("N");
    });
    test('executing ["b","b","b","b","b","b"] (checking boundary function working properly or not', () => {
